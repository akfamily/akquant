...
**状态:已落地(issue #400,`__engine_rule_version__` 1.5.5)**,但落地形态与这里的设想不同,差异值得记下:

- 上文把"`margin_delta <= 0` 时需求仅为佣金"当作**前提**,实际那正是 **bug 本身**——`margin_delta` 在算出后被 `.max(0)` 夹断,减仓释放的保证金被丢掉,需求才退化成"裸佣金"。
- 因此修法不是"给减仓单开一条豁免旁路",而是**不夹断增量、只夹断最终和**:`(margin_delta + commission).max(0)`。减仓释放的额度先把自己的费用抵掉,自然得到 0,无需特例分支。
- **两道闸门都改了**,不是只改提交闸门。这里设想的"执行时检查作为兜底"行不通:实测只修提交闸门时,拒单会原地转移到撮合器(`Insufficient margin at execution`)。
- 本节担心的"无持仓的 `Auto` 卖单(实为开空)仍被正确拒掉"已有护栏测试覆盖;现金账户另有 `StockAvailablePositionRule` 兜底,不会裸空。
