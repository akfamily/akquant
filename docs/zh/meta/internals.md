...
*   **动态风控**: 在成交前一刻按**真实成交价**再次检查账户资金（Free Margin），与提交时用最新价/限价预估的那道闸门**彼此独立**。资金不足时，若订单允许自动缩量（`allow_quantity_auto_resize`，`buy()` 默认允许而 `sell()` 不允许）则削减数量成为部分成交，否则拒单并在原因里带 `at execution` 标记。
    *   资金需求按**带符号**的保证金增量计算：`max(保证金增量 + 佣金, 0)`。减仓/平仓会**释放**保证金，增量为负，先抵掉自己的费用——所以卖单不需要预先持有现金，满仓账户也卖得出去（#400）。
    *   判定还要求 `total_required > 0`：可用资金允许为负（两融账户水下），否则任何零成本订单（包括此时唯一能自救的减仓单）都会满足"需求 > 可用"而被永久拒绝。
