...
1.  **计算保证金占用的变化量**（注意是**变化量**，不是订单金额）：
    *   股票: `Price * Quantity`
    *   期货: `Price * Quantity * Multiplier * MarginRatio`
2.  **计算费用**：预估佣金、印花税等。
3.  **比较**：`max(保证金变化量 + 费用, 0) > Free Cash` ?
    *   如果资金不足，订单会被**自动拒绝 (Rejected)**，或者根据配置**自动缩减数量 (Auto-resize)** 以适应剩余资金。

这里的"变化量"是**带符号**的，这一点决定了买卖两侧的行为完全不同：

*   **开仓/加仓**：变化量为正，需要现金预付，按上式验资。
*   **减仓/平仓**：变化量为**负**——平掉持仓会**释放**保证金。于是它先把自己的费用抵掉，`max(..., 0)` 通常得到 0，卖单**不需要预先持有现金**。这与真实 A 股规则一致：卖出的佣金、印花税、过户费都从成交款净额中扣除。

!!! warning "满仓后为什么仍然卖得出去"
    假设 100,100 元账户以 10 元买满 10,000 股，只剩几十元现金，而全量卖出的费用（佣金 + 印花税 + 过户费）合计上百元。费用**大于**剩余现金，但卖单不会被拒——因为卖出释放的 100,000 元完全盖得住这笔费用。

    若把费用当成现金预付来校验，这里就会拒单，且后果是账户**彻底冻结**：卖不出去 ⇒ 没有资金回款 ⇒ 后续调仓全部失效。这正是 issue #400 修掉的缺陷（`__engine_rule_version__` 1.5.5）。

    豁免有边界：它的语义是"费用从成交款净额中扣除"，而不是"平仓一律放过"。当卖出所得**盖不住**费用时（例如持 100 股 0.01 元的股票，所得 1 元而最低佣金 5 元），`max(..., 0)` 仍为正值，这单照常验资——放过它等于让现金透支，而只有 `check_cash=False` 才允许透支。

提交时与成交时是**两道独立的闸门**：提交时用最新价/限价预估（带 `safety_margin` 安全垫），成交时用真实成交价复算（不留安全垫）。因此偶尔会出现"提交通过、成交时被拒"的情况，拒单原因里带 `at execution` 标记。
