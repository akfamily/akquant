...
### 6.1.1 最小变动价位 (Tick Size) 与委托价校验

**最小变动价位 (Tick Size)** 是交易所规定的报价最小单位，委托价必须是它的整数倍。A 股股票为 `0.01` 元，基金/债券为 `0.001` 元。这不是惯例而是**硬约束**：非整数倍的委托会被交易所前端直接拒单。

初学者最容易在**计算价格**时踩坑——把均价、涨跌幅、目标市值反推出来的价格直接拿去下单：

```python
# 错误：算出来的价格几乎不可能正好落在 0.01 的整数倍上
price = self.get_history(20, symbol).mean() * 1.02   # 例如 2.8314
self.buy(symbol, 100, price=price)                   # 被拒单
```

AKQuant 对 `stock` / `fund` 缺省开启校验（`ChinaStockConfig.enforce_tick_size=True`），**回测撮合与实盘报单前都会拒单**而不是静默取整。这是有意的设计：若框架悄悄替你取整，回测与实盘会出现「同一笔单一边成交、一边被拒」的分裂，而拒单是显性的、你能立刻看到并处理。

正确写法是显式对齐，用 `Strategy.round_to_tick()`：

```python
price = self.round_to_tick(symbol, price, "down")   # 买入向下取整，不朝不利方向滑价
self.buy(symbol, 100, price=price)

# 卖出用 "up"，同样保守；"nearest" 为缺省，取最近的合法价位
```

方向的选择有讲究：**买入向下、卖出向上**是保守方向，代价是可能成交不了；反过来能提高成交概率，但等于每笔多付/少收一个 tick，是隐性成本。市价单（不传 `price`）不受此约束。

完整口径与逃生开关见 [tick size 对齐指南](../advanced/tick_size_alignment.md)。
...
### 6.4.4 费用从哪里出：买入需预付，卖出从净额扣

上面三项费用**买卖两侧的资金口径并不对称**，这一点直接决定了满仓策略能不能调仓：

*   **买入**：费用与买入金额一起构成资金占用，需要账户里**真有这笔现金**，否则委托被拒。
*   **卖出**：佣金、印花税、过户费都从**成交款净额**中扣除。卖出释放的资金远大于费用，因此卖单**不需要预先持有现金**。

所以"买满之后只剩几十元现金、而全量卖出的费用上百元"这种情形下，卖单**照常成交**——引擎校验的是保证金占用的**变化量**加费用，而平仓的变化量是负的。详见 [第 4 章](04_backtest_engine.md) 的 4.11.1 资金校验。

唯一的例外是卖出所得连费用都盖不住（例如持 100 股 0.01 元的股票，所得 1 元而最低佣金 5 元），这单仍会被拒——否则现金会被扣成负数。
...
- [AKQuant tick size 对齐指南](../advanced/tick_size_alignment.md) —— 最小变动价位的完整口径：回测撮合与实盘报单两侧的校验规则、`round_to_tick` 的三种方向、股票与期货两侧的开关，对应本章 6.1.1。
...
4. 委托被拒、提示「不是最小变动价位的整数倍」：计算得到的价格（均价、涨跌幅反推等）没有对齐 tick，用 `self.round_to_tick(symbol, price, "down")`（买入）/ `"up"`（卖出）对齐后再下单，见 6.1.1。
