...
2.  **Symbol 列**：**多标的数据必须带标的列**，否则所有标的的 bar 会被压进一条时间序列（指标与撮合结果都不正确）。列名可以是 `symbol`、`股票代码`、`code`、`ticker` 之一（AKShare 原生的 `股票代码` 直接可用，无需重命名）；推荐统一用 `symbol`。识别不到标的列、却检测到同一时间戳存在多行时，回测会打 `WARNING` 提示已退化为单标的。单标的数据可以省略该列（此时引擎按单一标的处理，不会告警）。
...
*   `get_history` / `get_history_multi` / `get_history_df` / `get_rolling_data` 可用，返回**成交价序列**：tick 有自己独立的一条历史序列（不再与 bar 共用缓冲区），省略 `freq`（纯 tick 场景没有歧义，不要求显式指定）等价于 `freq='tick'` 的默认取数路径，`get_history(count, symbol, "close")` 等价于取最近若干笔成交价。
    该独立序列内部仍按 `open=high=low=close=price` 存储（历史包袱：与 bar 共用同一存储结构），但**只有显式传 `freq='tick'` 时**才会在 Python 层校验 `field`——此时请求 `open`/`high`/`low` 会抛 `ValueError`（tick 没有真正的高低开盘价，可用字段为 `price`/`close`/`volume`）；`get_history_df` / `get_rolling_data` 固定取 OHLCV 五字段，故显式传 `freq='tick'` 调用它们必然报错，纯 tick 场景请省略 `freq` 或改用 `get_history(freq='tick', field='price')`。若该 symbol 同时存在 bar 来源（见下面的 `freq` 聚合，或 [API 参考](../reference/api.md) 中 `get_history` 关于 `freq` 参数的双流说明），省略 `freq` 时按当前所处的回调自动定档（`on_bar` 里取 bar、`on_tick` 里取 tick）；在行情回调之外（如 `on_timer`）则因两条序列并存而报错，要求显式传 `freq='tick'` / `freq='bar'`。
*   增量指标（`self.I(aq.SMA(20), ...)` 这类有 `update()` 的对象）的**单值模式**可用：`source` 取 `open`/`high`/`low`/`close` 均返回成交价，取 `volume` 返回单笔量；`close_volume` 模式同样可用。
...
*   输入含任意 `Tick`（**纯 tick 或混合皆然**）+ 策略里**声明过向量化预计算指标**（即 `self.I(Indicator(...))`）会抛 `ValueError`：归一后走 `DataFeed` 分支，该分支不构建预计算指标所需的 DataFrame。护栏有两条判据，都满足才报错：一是「是否有 tick」（与是否同时有 bar 无关），二是「是否真的声明过向量化预计算指标」（只传增量指标的策略不受影响）。报错文案为「已声明的向量化预计算指标不支持含 Tick 的输入: ...请改用增量指标 self.I(aq.SMA(20), ...)...」。需要指标时改用增量指标，或用下面的 `freq` 把 tick 聚合成 bar。
