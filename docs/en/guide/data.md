...
*   `get_history` / `get_history_multi` / `get_history_df` / `get_rolling_data` all work and return a **series of trade prices**: ticks now live in their own history series (no longer sharing a buffer with bars), so omitting `freq` — unambiguous in a ticks-only session, so it is not required — is equivalent to the `freq='tick'` read path; `get_history(count, symbol, "close")` gives you the most recent trade prices.
    That series is still stored internally as a degenerate bar (`open=high=low=close=price` — a storage-layer holdover from once sharing the bar layout), but `field` is only validated in Python when you pass `freq='tick'` explicitly: requesting `open`/`high`/`low` then raises `ValueError` (a tick has no real high/low/open — the usable fields are `price`/`close`/`volume`). `get_history_df` / `get_rolling_data` always pull the full OHLCV set, so calling them with an explicit `freq='tick'` always raises; in a ticks-only session, omit `freq` instead, or use `get_history(freq='tick', field='price')`. If the symbol also has a bar source (see the `freq` aggregation below, or the dual-stream notes for `get_history` under `akquant.Strategy` in the [API reference](../reference/api.md)), omitting `freq` resolves by the callback you are currently in (bars inside `on_bar`, ticks inside `on_tick`); outside the market-data callbacks (e.g. `on_timer`) it raises because both series coexist — pass `freq='tick'` / `freq='bar'` explicitly.
*   Incremental indicators (any object with `update()` passed to `self.I(...)`, e.g. `self.I(aq.SMA(20), ...)`) work in **single-value** mode: `source` of `open`/`high`/`low`/`close` all return the trade price, `volume` returns the per-trade volume; `close_volume` mode works too.
...
*   Any input containing a `Tick` (**ticks-only or mixed alike**) combined with a **declared vectorized precomputed indicator** (i.e. `self.I(Indicator(...))`) raises `ValueError`: normalized tick input flows through the `DataFeed` branch, which does not build the DataFrame precomputed indicators need. The guard has two conditions and both must hold: "is there a tick" (independent of whether bars are present too) and "was a vectorized precomputed indicator actually declared" (strategies that only pass incremental indicators are unaffected). Use incremental indicators instead — the error message points at `self.I(aq.SMA(20), ...)` — or aggregate with `freq`.
...
*   **Configuration**: Set `warmup_period = 60` in the strategy. The engine lets **each symbol accumulate its own** 60 bars solely for updating indicators, without triggering `on_bar` trading logic during that window.
*   **Multi-symbol**: the threshold is counted **independently per symbol** — symbol A starts trading as soon as it has accumulated 60 bars on its own, without waiting for symbol B. Because of this, set `warmup_period` directly from the indicator window (e.g. `self.warmup_period = self.params.long_window + 1`); it does **not** need to be multiplied by the number of symbols.
