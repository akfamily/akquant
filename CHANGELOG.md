...
### Added
- **`run_live()` 新增 `session_tag=` 参数，支持多任务交易同一标的时的委托回报隔离**：0824 反馈第 1 点——平台多个任务交易**同一标的**时，各任务此前会收到彼此的 `on_order`/`on_trade`/`on_execution_report` 推送（v0.3.50 已解决「交易不同标的」的隔离，同标的这条此前判定为「需柜台在推送帧补任务标识才能解决」，该判定是错的）。条件其实已经具备：本框架生成的 `client_order_id` 格式是 `{broker}-{session_tag}-{seq}`，`session_tag` 默认随机 8 位 hex；中间件推送帧本就带 `client_order_id` 回传——比对前缀即可判定归属，**不需要改中间件**。传入 `session_tag`（推荐传平台任务 ID）后替代随机值，且**同时启用严格拒绝**：前缀不匹配的推送会被主动丢弃，计入新的 `foreign_task` 计数（与既有的 `foreign_symbol` 分开计，见下方 Changed）。校验 fail-fast：只允许 `[A-Za-z0-9_]`、长度 ≤ 32、不能含 `-`（它是 `client_order_id` 的字段分隔符），违反直接 `raise ValueError` 并给出可照抄的正确写法，不做静默归一化改写。自定义 broker 要支持该能力，必须在推送帧里**原样回传** `client_order_id`（不截断、不改写字符），详见 `docs/zh/advanced/adding_a_broker.md`「委托回报的标的过滤」一节。
...
### Changed
- **委托回报的标的过滤新增会话标记判据，优先级高于原有的订单归属/标的判据**：`gateway/broker_event_bridge.py::_accepts_symbol` 之前只有两级（订单归属映射 → 标的白名单），现在前面加了两层——`client_order_id` 前缀命中本会话 `{broker}-{session_tag}-` 一律放行（即便标的不在挂载列表内，覆盖 `BrokerOrderSink` 经外部信号源直调下单能合法报出挂载集合之外标的的场景），前缀不匹配且**显式传了 `run_live(session_tag=...)`**（严格模式）时主动拒绝，计入独立的 `foreign_task` 计数（与 `foreign_symbol` 分开计：前者是多任务稳态下必然增长的正常工作量，后者异常增长才指向标的配置错误）。**未传 `session_tag` 时严格拒绝完全不生效**，行为与升级前逐字一致——这是刻意的向后兼容防线：若柜台截断或改写 `client_order_id`，本会话自己的单前缀也会不匹配，无条件拒绝会把自己的回报全部吞掉（现象是「下单成功却收不到回调」）。会话收尾摘要新增一行 `Dropped (foreign task)`；盘中周期性丢弃计数汇总（`_report_dropped_event_counts_if_changed`）的触发条件**不变**，仍只看 `foreign_symbol`，刻意不把 `foreign_task` 纳入，避免重现此前刚修掉的「稳态刷屏」。
