...
- **声明式指标层：`Strategy.I()` 一次声明 = 自动增量更新 + 序列回溯 + 自动绘图（对标 TradingView Pine Script，**破坏性变更**：删除 `indicator_mode` 与两个 `register_*` 方法）**。此前指标能力分散在三套互不相通的机制里：`register_precomputed_indicator` 的向量化预计算（实盘用不了，没有完整 DataFrame）、`register_incremental_indicator` 的有状态增量、以及在 `on_bar` 里 `get_history(N)` 全量重算；前两者还被 `indicator_mode` 这个**互斥**开关锁死，只能二选一。更关键的是**计算与绘图完全脱钩**——算出一个值和把它画出来是两件毫不相干的事，用户得在每根 bar 手写 `record_indicator` 并重传一遍 pane/color 等元数据；而指标本身只暴露 `.value`，判断金叉这种最基本的形态还要自己维护 `deque`。
  ...
  **迁移**：`self.indicator_mode = "incremental"` + `register_incremental_indicator("x", obj, source="close")` → `self.x = self.I(obj, name="x", source="close")`；`register_precomputed_indicator("x", ind)` → `self.x = self.I(ind, name="x")`；`indicator_factory=f` → `factory=f`。`StrategyRuntimeConfig.indicator_mode` 与 `BacktestConfig.indicator_mode` 两个字段一并删除。`record_indicator` **保留**为底层逃生口，用于记录非指标类的自定义值（仓位、信号强度、风控中间量）。**已知约束**：热启动下 `factory` 不能是 lambda（快照要 pickle 策略实例，局部对象序列化不了），须用模块级函数 + `functools.partial`；预计算路径实盘用不了、含 Tick 的输入会报错、不支持 `freq=`。声明位置 `__init__` 与 `on_start` 都支持，但**推荐 `on_start`**——`self.params` 在 `__init__` 之后才注入，参数化指标（`run_grid_search` 要扫的那些）只有在 `on_start` 里才拿得到参数值。
  Golden 基线**数值零变化**，即删除 `indicator_mode` 未波及任何回测行为。新增 `tests/test_indicator_declaration.py` 18 条（序列回溯与越界、多值 outputs 拆分与分量回溯、plot 触发条件、预热期不上报、per-symbol 状态隔离、共享实例跨标的 fail-fast、`freq=` 绑定窗口周期、`lookback` 有界、预计算与增量共存）。三次回退验证确认测试非恒真：破坏 `lookback` 有界、破坏越界保护、破坏分量下标映射，各自被对应测试抓出——其中"分量下标恒取 0"**最初未被抓到**（断言只验了第 0 个分量 `dif`，而它恰好就是下标 0，恒真），补强为三分量全验 + 分量互不相等后才抓住。指标事件流同时新增 `confirmed` 字段(`STREAM_SCHEMA_VERSION` 升至 `1.3`):当前恒为 `true`(指标都在 bar 闭合后计算),提前占住字段位是为了让后续"未闭合 bar 的实时指标"能发 `false` 而**不必升 MAJOR**——前端一旦按"无此字段 = 永远确认"写死,再加就是破坏性的;消费端约定同 `(indicator_key, symbol, timestamp)` 的后来者覆盖先前者。设计与后续 L2/L3/L4 路线见 [RFC](docs/zh/meta/indicator-tradingview-rfc.md),示例 `examples/72_declarative_indicators.py`。
- **LWC 复盘补指标渲染 + 实时增量适配(`result.viz.review(include_indicators=True)`、`akquant.lwc.to_lwc_update()`)**。此前 `akquant.lwc` 的静态复盘完全不画指标(`grep -i indicator lwc/` 零命中)——刚落地的 `self.I()` 自动上报的点,在 LWC 图上根本看不到,这是比"实时"更基础的缺口。新增 `lwc/_indicators.py` 作为 `chart/d3kline.py` 的镜像:同一份指标契约(7 值 `render_type`、整数 `pane`、`timestamp_ms`)面向第二个具体前端 lightweight-charts v5。映射:`line→Line`、`area→Area`、`bar/column/histogram→Histogram`、`scatter→Line(lineVisible=false, pointMarkersVisible=true)`、`signal→` 主图 K 线 marker(不建 series,它的契约本就是"标记类渲染")。**pane 映射刻意 +1**:akquant `pane 0` 叠主图 → LWC pane 0,akquant `pane k≥1` → LWC pane **k+1**——LWC pane 1 已被成交量占用,量纲不同的指标不该被成交量压扁;前端按出现顺序把不连续的 pane 压实,用户只声明 `pane=3` 也不会撞上 LWC 的连续下标要求。时间值与 K 线 candles 同一套(日频 `YYYY-MM-DD` / 日内 UTC 秒),否则画不到同一根 bar 上;点位丢 `None/NaN`、按 time 去重(留最后)+ 排序——LWC `setData` 对乱序/重复 time 直接报错。`reference_lines` 透传为 `priceLine`;无 `color` 的指标按主题色板轮转,明暗切换只 `applyOptions` 重上色不重建数据。`include_indicators` **默认 True**:复盘看不到自己声明的指标才反直觉。实时路径:`to_lwc_update()` 吃 `to_indicator_message()` 的 point 消息产出 `series.update()` 可直接消费的增量,`confirmed` 原样带出——临时点与确认点同 time,LWC `update()` 按 time 覆盖,未闭合 bar 实时指标(L2)落地后前端零改动;新增公开 `load_lwc_js()` 供自建页面内联。**HTTP/WS 传输刻意不进核心**(守 viz RFC),闭环演示放在 `examples/73_lwc_live_indicators.py`:`run_live(broker="replay")` + stdlib `http.server` 轮询 + LWC 惰性建 series,实测 60 根 K 线 / 174 条增量 / 3 个指标 key,exit 0。新增 `tests/test_lwc_indicators.py` 17 条;5 项回退验证均抓住(pane 不偏移、去重留首个、忽略 `include_indicators=False`、`draw()` 不调 `drawIndicators`、NaN 不过滤)。模板测试刻意断言 `curSignals = drawIndicators(s)` 这一调用点而不只是函数存在——光有定义没被调用,指标 payload 就是白带的。
- **可插拔 study:`akquant.Study` + `run_backtest(studies=[...])` / `run_live(studies=[...])` + `indicator_df(owner=)`**。对标 TradingView 里 indicator 脚本与 strategy 脚本的分野:一张图叠 N 个只画图的单元。先做 spike 证明**多 slot 拓扑今天就能承载它**——只声明 `self.I()` 的类通过 `strategies_by_slot` 挂进去,回测与实盘 replay 都产出带独立 `owner_strategy_id` 的指标点、主策略交易不受影响——所以 L3 **不新建管线**,增量只有护栏与入口糖:`Study(Strategy)` 把 19 个交易 API(`buy`/`sell`/`short`/`cover`/`submit_order`/`place_*`/`order_target*`/`rebalance_*`/`close_position`/`cancel_*`)全部换成抛 `StudyCannotTradeError`,这是**硬保证而非约定**——study 在 `on_bar` 里误写 `self.buy` 会在引擎回调里当场报错,不会静默成交;`studies=` 在风控校验**之前**经 `merge_studies_into_slots()` 并入 `strategies_by_slot`,让后续所有按 slot key 的校验/归属/窗口订阅合并对它一视同仁,key 取 `study_id`(省略为类名 snake_case),撞 `strategy_id` 或已有 slot key 抛 `ValueError`、混入非 `Study` 抛 `TypeError`(否则"不交易"的保证就没了);`indicator_df()` 新增 `owner=` 过滤。新增 `tests/test_study.py` 27 条(19 个交易 API 参数化拦截 + id 生成/冲突/类型校验 + 回测与实盘 replay 端到端 + 引擎循环内误下单冒错);5 项回退验证均抓住(不装拦截、不查冲突、不查类型、owner 过滤失效、实盘门面不合并)。示例 `examples/74_pluggable_studies.py`。
- **窗口周期指标的实时值(intrabar):`self.I(..., freq="5min", intrabar=True)` + `binding.confirmed` + 流事件 `confirmed=false`;新增 Rust `clone_indicator()`**。对标 Pine 的 `barstate.isrealtime` / `isconfirmed`:每根基础 bar 闭合时,用 `ctx.current_window()` 的未闭合窗口快照在指标**副本**上试算一个临时值,真实状态不受污染;窗口闭合时真 `update()`。下标语义与 Pine 完全一致——有临时值时它占据 `[0]`,已确认序列整体后移一位(`[1]` 是上一根已确认)。临时点只走流事件、**不进** `indicator_df()` / `export_indicators()`;其时间戳就是该窗口将来闭合的标签(基础周期已知时,Rust `WindowAggregator::current` 的快照 `timestamp == label`),确认点随后以同一 `time` 到达,前端按 `(indicator_key, symbol, timestamp)` 覆盖——正是 `STREAM_SCHEMA_VERSION` 1.3 预留 `confirmed` 字段位时定下的契约,L4 的 `to_lwc_update()` 零改动。**`IndicatorSink` 协议同步新增 `record(..., confirmed: bool = True)` 参数**,但对按 1.2 版签名实现的第三方 sink **向后兼容**:`Strategy.record_indicator` 先用 `inspect.signature` 探测 sink 是否接受 `confirmed`(显式参数或 `**kwargs`,按 sink 缓存),不接受则确认点照常送达、临时点跳过并告警一次——旧 sink 分不清临时点,给它只会在图上留下永不被覆盖的脏点。内置 `IndicatorRecorder` / `StreamingIndicatorSink` 已同步。默认 `intrabar=False`,既有行为零变化。**RFC 里「clone()+update() 零改动」的假设被 spike 推翻**:Rust 侧确有 `#[derive(Clone)]`,但 pyo3 不会因此暴露 `__copy__`,`copy` / `deepcopy` / `pickle` 对全部 103 个内建指标一律 TypeError。最终只加**一个** pyfunction `akquant.clone_indicator(obj)`(72 个类型的 `cast` 链由宏生成,非内建指标抛 TypeError 指向 `copy.deepcopy`),Python 侧 `peek_indicator = clone → update → value`;用户自写 Python 指标走 `deepcopy`。`akquant.pyi` 经 `cargo run --bin stub_gen` 重新生成。**范围刻意收在窗口周期指标**:`intrabar=True` 而 `freq=None` 直接 `ValueError`。基础 bar 在 tick 上的实时值后置——它要给 `ctx` 暴露 `BarAggregator` 的未闭合 bar,更要先裁定双流下 tick 是否继续 `update()` 基础指标(纯 tick 策略依赖现状),那是破坏性选择。基础周期未知时窗口标签随每根基础 bar 漂移,只算 `[0]` 不发流事件并告警一次。实施中挖出并修掉两个 bug:① **跳窗**——引擎顺序是 `on_bar` 先、`on_window_bar` 后,闭合那根基础 bar 上 `current_window()` 已是下一窗口而上一窗口确认值未入账,照常试算会漏掉一整根(流里表现为 `09:45 …false` 出现在 `09:40 …true` 之前);修法按快照标签跟踪,标签变了但确认未到就跳过这一帧。② **L1 遗留**——窗口指标确认点的时间戳取自 `current_bar`(基础 bar),而 `on_window_bar_event` 里指标推进发生在 `current_bar = 窗口 bar` 之前;正常闭合时两者恰好相等而被掩盖,尾部 flush 时窗口标签(ceil)≠最后一根基础 bar 就露馅。修法是上报时显式传事件自身的 `timestamp`。新增 `tests/test_indicator_intrabar.py` 14 条(peek 不污染 Rust/Python 指标、多值 peek、intrabar 需 freq、回测 tick→1min→5min 端到端的 `[0]`/`[1]`/`confirmed`/流序列/DataFrame 只含确认点/默认关时行为不变、确认点时间戳=窗口标签、实盘 replay 端到端);5 项回退验证均抓住(peek 污染原对象、不跳过窗口滚动帧、`[0]` 不返回临时值、临时点进 DataFrame、确认点时间戳退回 `current_bar`)。示例 `examples/75_intrabar_indicators.py`。**同轮补齐基础周期(Tier B):`intrabar=True` 不再要求 `freq=`,基础周期指标在每笔 tick 上试算。**此前认为它有两个前置问题,实施时都绕开了:① 不需要 Rust 暴露 `BarAggregator.active_bars`——spike 证实双流回测里同一分钟的 tick 先到、闭合 bar(区间末 −1ns 打戳)后到,于是 Python 侧从 tick 自己累一根 `PartialBar`(真实 open/high/low/close,bar 闭合即重置),H/L 类指标(ATR)在 tick 上因此**首次**能算出值(纯 tick 路径只能跳过);临时点时间戳用聚合器自己的公式 `end_stamp_label = (ts // 间隔 + 1) * 间隔 − 1ns` 直接预测——双流下 bar 必须打区间末戳(否则与 tick 混推时间戳倒退),公式按绝对时间取整,跨午休/跨日成立;bar 真闭合时与 `bar.timestamp` 比对**自校验**,不符(行情源按起点打戳)则该 symbol 停发临时点并告警一次,`[0]` 仍可读。② 不需要裁定破坏性语义——tick 从 `update` 改为 `peek` **只对显式 `intrabar=True` 的基础周期指标生效**;默认 False 时 tick 照旧 `update()` 基础指标,纯 tick 策略零变化(有测试钉住)。已知尾部现象:回测数据末尾最后一笔 tick 开出的 bar 永不闭合(没有下一笔 tick 触发),它的临时点没有确认点;实盘下一笔 tick 就会闭合。另一个实施中确认的事实:`run_backtest(warmup_period=0)` 压不住源码推断的 warmup(`SMA(2)` → 2),首根 bar 的 `on_bar` 会被吞而指标照常更新——测试期望因此改为从 tick 序列推导而非从 `on_bar` 记录反推。新增 8 条测试(允许无 freq / 拒绝预计算 / 标签公式与 Rust 打戳一致 / tick 临时值 = SMA(上一根 close, 最新价) / bar 闭合确认且状态未被 tick 污染 / `[1]` 为上一根确认 / 流标签与闭合 bar 同 time / DataFrame 只含确认点 / ATR 的 H/L 试算 / 默认关时 tick 照旧驱动)。示例 `examples/76_tick_intrabar_indicators.py`。
- **声明式指标层 code-review 收口(同轮)**:① 省略 `name` 的 `self.I()` 自动命名改为**按声明顺序计数**(首跑记下 `on_start` 起点的计数,恢复时拨回),此前"查字典是否已占用"在 warm start 恢复时永远追加后缀 `_2`,让恢复的预热状态被孤立、用户拿到冷启动的 binding;② pickle 改为按**持有 binding 的属性名**重建(`_indicator_binding_attrs`),此前 `__setstate__` 按声明名 `setattr` 会覆盖用户的同名属性(`self.period = 20` 被换成 binding),且 `__getstate__` 那段删除逻辑对 `self.I()` 返回值赋到别名属性的写法是死码;③ 向量化预计算 `Indicator` 传 `warmup_bars>0` 直接 `ValueError`(它没有增量状态,此前会在数据准备阶段撞 `Indicator.update` 的 `NotImplementedError`),bootstrap 循环同时跳过 `precomputed`;④ 预计算 `Indicator` 自己按 symbol 缓存整段结果,多标的下 `get_instance()`/代理属性不再套"共享实例不可跨 symbol"校验(此前第二只标的直接抛错,而 `[0]` 却正常,同一 binding 两个属性行为不一致),报错文案里过期的 `indicator_factory` 改为 `factory=`;⑤ 纯 tick 会话(无 bar)开 `intrabar=True` 的基础周期指标会永远只试算不确认,且试算分支绕过了 H/L 覆盖率记账——现在记 `intrabar_symbols`,会话末与 `bar_seen_symbols` 做差报 `StrategyConfigurationError`,H/L 模式同时补记 `tick_only_symbols`;⑥ 没有任何基础周期 `intrabar` 声明时不再逐 tick 维护 `PartialBar`(`_has_base_intrabar` 标志),纯 tick 高频策略零额外开销;⑦ 实盘 sink 的快照级 `confirmed` 判定改为复用 `indicator_recording.all_items_confirmed`,两侧口径不再各写一份。新增 `tests/test_indicator_declaration_review.py` 7 条,每条对应一个 finding,修复前全部失败;4 项回退验证(计数不回拨、预计算套共享实例校验、不记 `intrabar_symbols`、临时点仍推给旧 sink)均抓住。同时补上**实盘多 slot 各自订阅不同窗口周期**的测试(`tests/test_live_multi_slot_windows.py`,4 条):主策略 15min(全标的)+ study A 5min(全标的)+ study B 5min(只限一个标的)在 `broker="replay"` 下同跑——B 与 A 同周期、标的范围重叠但不完全相同,正是 `_merge_scoped_specs` 必须合并成一条 spec 的场景,不合并会被 Rust 侧拒绝、会话起不来。实盘 runner 走的是与回测同一个 `configure_engine_window_subscriptions(engine, targets, ...)`,此前没有测试钉住 `targets` 含全部 slot。断言各 slot 只收到自己订阅的周期、B 收不到第二只标的、窗口回调次数与 5min/15min 网格逐一对上(含尾部 flush)、流事件里各 slot 指标点带自己的 owner 且点数与窗口回调数一致。3 项回退验证均抓住:只下发主策略订阅(3 败)、窗口派发不按 `symbols` 过滤(1 败)、同周期重叠订阅不合并(4 败,会话起不来)。
- **引擎原生多周期：`Strategy.subscribe_bars(freq)` + `on_window_bar`，同一 symbol 可持有多条周期序列（`__engine_rule_version__` 升至 1.6.0）**。此前只有 `feed.resample`/`feed.replay` 的伪标的拼接与 `BarGenerator` 策略内运行时聚合两条组合式路径，`get_history(freq=)` 的取值域此前"为多周期留了口子"但一直没有真正落地。现在窗口聚合由 Rust `WindowAggregator` 从基础 bar 直接完成，窗口 bar 不进 feed、不参与撮合；`get_history(count, symbol, field, freq=)`/`get_history_multi(...)`、`register_incremental_indicator(..., freq=)`、`current_window(symbol, freq)` 均可读取已订阅的窗口周期；`Bar` 新增 `freq` 字段标注其所属周期。窗口闭合分两种口径：基础周期已知（`run_backtest(freq=)` 或实盘/replay 网关声明 `metadata["freq"]`）时零延迟闭合，未知时晚一根基础 bar 闭合并打一次性 `WARNING`。日线窗口的标签取**当日最后一根基础 bar**（与 `BarGenerator` 的"次日零点" pandas 口径刻意不同，两条路径不能混用同一时间戳假设）。同一周期、标的范围重叠但 `session_windows` 不一致的订阅在引擎配置阶段抛 `ValueError`。`get_history`/`get_history_multi` 在同一 symbol 同时存在多条历史序列（如 bar+tick、或多个窗口周期）且未显式指定 `freq` 时，报错文案改为"同时存在多条历史序列"以覆盖窗口场景。checkpoint 新增 `history_window_series` 标记；用旧版本存档 `run_from_checkpoint` 时会告警提示窗口序列特征缺失。**取舍与限制**：`freq` 仅支持整数分钟/小时与 `1d`，不支持秒级/周/月周期；`extra` 字段不参与窗口聚合；窗口 bar 从不进入撮合（仍需在基础行情回调里下单）；CTP 网关合成 bar 默认打区间起点戳，多周期聚合需要同时开启 `emit_ticks` 与 `emit_bars` 才能拿到对齐的结束戳；会话结束 flush 尾部未闭合窗口后若续跑 checkpoint，尾部区间会重新形成一根同标签窗口（历史里出现两根同标签 bar，是已知且刻意保留的行为）。详见 [多周期策略：三条路径](docs/zh/advanced/multi_timeframe_feed_api.md)、示例 `examples/14_multi_frequency.py`（回测）与 `examples/71_native_multi_timeframe_live.py`（实盘/replay）。
...
- **纯减仓卖单的手续费不再要求现金预付，费用从成交款净额中抵扣（`__engine_rule_version__` 升至 1.5.5，issue #400）**：满仓策略在现金趋零后，所有卖单被 `Risk: Insufficient margin` 拒绝——实测 100,100 元账户以 10 元买入 10,000 股后余现金 68 元，全量卖出费用合计 132 元，卖单提交即被拒（`Required: 132.00000, Available: 67.9932`）。后果是**账户彻底冻结**：卖不出去 ⇒ 没有资金回款 ⇒ 后续调仓全部失效。这与真实 A 股规则相悖：卖出的佣金、印花税、过户费都从成交款净额中扣除，卖出永远不需要预先持有现金。
  ...
  提交闸门与成交闸门是**两道彼此独立**的检查，必须同步修改：实测只修 `risk/common.rs` 时，拒单会原地转移到撮合器（`Risk: Insufficient margin at execution`）。
  同时修掉一个**独立的、更容易撞上的缺陷**：成交闸门的判定新增 `total_required > 0` 前置条件。可用资金允许为负（两融账户水下），此前任何**零成本订单**都满足 `required > free_margin`——包括此时唯一的自救手段减仓卖单——被以 `Required: 0, Available: -19943` 这种自相矛盾的理由**永久**拒绝。该场景与手续费无关，且所有费率取库默认值 0 即可复现。
  **豁免的边界刻意不放宽**：语义是"费用从成交款净额中扣除"，而非"平仓一律放过"。卖出所得盖不住费用时（如持 100 股 0.01 元的股票，所得 1 元而最低佣金 5 元），`required` 仍为正值并照常验资——放过它等于让现金透支，而只有 `check_cash=False` 才允许透支（#280）。另有两点保持原状：卖出超过持仓的订单仍是**全单拒绝**而非拆成"减仓部分放过 + 反向开仓部分验资"（现金账户由 `StockAvailablePositionRule` 兜住，不会裸空）；`risk/manager.rs` 的提交时自动缩量仍为买入侧独有。
  Golden 四个基线**数值零变化**，即本次改动未波及既有的期货、期权、撤单与 T+1 路径。新增 5 个 Python 端到端测试（主场景成交、水下账户减仓成交、费用超所得仍拒、买入侧仍拒、现金账户不裸空）与 3 个 Rust 单测（现金账户与两融账户两条 margin 路径各一，外加费用超所得的拒单护栏）。四处回退验证均确认测试非恒真：恢复 `common.rs` 夹断、恢复 `simulated.rs` 夹断、去掉 `total_required > 0`、以及"卖单一律放过"的过度修复，各自都被对应测试抓出失败。
- **指标事件流补齐 `timestamp_ms`，三个消费出口的字段口径重新一致（`STREAM_SCHEMA_VERSION` 升至 `1.2`）**：`record_indicator` 的契约是同一次调用在三个出口——stream 事件、`indicator_df()` DataFrame、`export_indicators()` JSON/parquet——字段与编码保持一致。采集层四个发射点（回测 `indicator_recording.py` 的 point/snapshot、实盘 `live/_stream_sink.py` 的 point/snapshot）**都已填** `timestamp_ms`，但桥接函数 `to_indicator_message()` 的两个分支在组装消息时**双双漏掉了这个字段**，只透出纳秒 `timestamp`。于是 DataFrame 与 export 两个出口有毫秒、stream 出口没有，契约在流式路径上是破的。
  影响面正落在这个字段存在的唯一理由上：`timestamp_ms` 就是为"前端图表库直接可用、无需换算单位"而加的，而需要它的恰恰是 stream 这条实时路径（批量路径读 DataFrame 本来就有）。因此每个消费 WebSocket / SSE 指标流的前端都得自己写一次 `// 1_000_000`，而这正是该字段想消除的样板代码；纳秒值在 JavaScript 里还超出 `Number.MAX_SAFE_INTEGER`，换算前后都有精度隐患。
  修法是在 point 与 snapshot 两个分支各透出 `timestamp_ms`，并对**只带纳秒的 payload 自动派生**而不是填 0：早期版本抓下来的事件、以及第三方 `IndicatorSink` 实现都可能只填 `timestamp`，派生口径与 `BacktestResult._indicator_points_df` 给历史 payload 兜底的 `fillna(timestamp // 1_000_000)` 完全一致。payload 里有显式值时以显式值为准。snapshot 的 `items[]` 内**不重复**该字段——一个 snapshot 内所有 item 共享同一时刻，snapshot 级别给一次即可。
  `STREAM_SCHEMA_VERSION` 按既有约定升 MINOR（`1.1` → `1.2`，与 `fill.position_effect` 那次同类）：前端靠信封里的 `schema_version` 协商可选字段，不升版则无法判断 `timestamp_ms` 是否该存在。纯新增字段、向后兼容，旧消费者继续读纳秒 `timestamp` 即可，无需改动。
  新增 4 个回归测试：回测流的 point/snapshot 均带该字段且等于 `timestamp // 1_000_000`、旧 payload 能派生、显式值优先于派生、schema 版本不低于 `1.2`。前三个均已确认在修复前复现失败（`KeyError: 'timestamp_ms'`），非恒真断言。另用 `run_live(broker="replay", trading_mode="paper")` 实跑实盘路径，确认实盘桥接消息同样带该字段、且与回测同一批 bar 的取值一致。
- **回测结果确定性修复：相同数据不同 symbol 顺序现在产生相同的回测结果（issue #211）**：当以 `Dict[str, DataFrame]` 形式传入 `data_map` 时，不同的标的遍历顺序会导致相同时间戳的 bar 以不同顺序进入引擎，虽然按时间戳排序是稳定的（保持插入顺序），但相同时间戳的不同标的 bar 的处理顺序不同，在有资金/持仓约束时可能导致不同的撮合结果和指标。修复方式是在 `BarColumns::sort_by_timestamp()` 和 `SimulatedDataClient::sort()` 中增加**二级排序键（symbol 字典序）**：时间戳相等时按 symbol 排序，确保回测结果的完全确定性。此修复确保了：① 无论 `data_map` 中 key 的顺序如何，相同数据总是产生完全相同的回测指标（`total_return`、`sharpe_ratio`、`max_drawdown` 等）；② 成交记录的数量和内容完全一致；③ 不影响单标的回测或已按 symbol 排序的多标的数据。
- **T+1 下 `NextOpen` 卖单不再按下单时刻校验可用持仓，改为按实际成交时刻（`__engine_rule_version__` 升至 1.5.4，issue #391）**：`t_plus_one=True` + 默认 `NextOpen` 成交策略下，在 `on_cross_section`（当根 bar 收盘时刻）提交的卖单会在**下一根 bar 开盘**成交，但可用持仓风控读的是**下单时刻**的账户快照。若卖的是前一交易日买入的持仓，该快照里它仍被 T+1 锁定（`Available: 0`），卖单当场被拒——尽管等到它真正成交的次日开盘，日终结算早已把这笔持仓解禁。这也与真实 A 股规则相悖：T 日收盘后为 T+1 交易日挂的卖单，对 T 日买入的股票是合法委托。
  影响面不是边缘场景而是**整条日频调仓链路**：`on_cross_section` + `rebalance_weights` / `rebalance_to_topn` 只要有换手，就会有大量标的只持有一天，每个这样的标的在卖出时都被拒；卖出腿被拒 ⇒ 没有资金回款 ⇒ 配对的买入腿紧接着以 `Insufficient margin` 被拒，组合直接无法调仓。实测复现脚本里 `t_plus_one=True` 产生 2 笔拒单（1 卖 + 1 连锁买），`t_plus_one=False` 则 0 笔——这个"关掉 T+1 就好了"的现象正是问题被发现的方式。
  修法是把该规则的**校验时点**对齐到成交时点，而不是放宽规则：`RiskRule` 新增 `defer_for_delayed_execution()`，`StockAvailablePositionRule` 声明选入；提交时若订单是**股票/基金的卖单**且成交策略为延迟撮合（`bar_offset >= 1`），跳过提交时校验，改由 `execution::simulated` 在撮合时按成交时点的可用持仓复查。撮合侧维护一份 slice 内逐笔推进的 `projected_available_positions`，**同一 slice 内的多笔卖单会累计扣减**，因此延后校验不会退化成"不校验"——裸卖（从未买入）与累积超卖（一份持仓卖两次）都仍会被拒，只是拒单时点从当根收盘挪到次日开盘。
  **三处边界刻意不放宽**：① `bar_offset == 0`（`CurrentClose` 等同周期成交）提交即成交、中间没有结算，仍按下单时刻校验，日内 T+0 转卖照旧被拒；② 买单不查可用持仓，不进延后路径；③ **实盘不延后**——柜台在报单时就要校验，且实盘没有模拟撮合器来复查，故延后开关仅由回测通道（`!execution_model.is_live()`）开启，公开的 `RiskManager.check_and_adjust` 保持提交时校验语义不变。
  **一处行为变更需注意**：受影响卖单的拒单时点后移一根 bar，`on_reject` 回调因此更晚触发，拒单原因文案也从 `Insufficient available position for X. Available: ...` 变为带 `at execution` 标记的 `Insufficient available position for X at execution. Available: ...`——按原文案做字符串匹配的策略需同步调整。
  Golden 基线 `stock_t1` 相应更新：该用例原有 2 笔拒单中，**「01-02 挂单 / 01-03 成交」与「01-04 挂单 / 01-05 成交」两笔转为成交**（持仓在成交时点确已解禁），而策略注释明确期望失败的那笔**日内 T+0 转卖仍为拒单**——关键不变量未被破坏。另外三个基线（`futures_margin` / `option_basic` / `order_cancel`）仅版本元数据变化，`total_commission` / `net_pnl` / `end_market_value` 全部数值不变，即本次改动未波及期货、期权与撤单路径。新增 4 个 Python 回归测试（调仓不再被拒、裸卖仍拒、累积超卖仍拒、同周期策略仍在提交时拒）与 4 个 Rust 单测（公开 API 保持提交时校验、延后路径放行、同周期不延后、买单不延后）；前三个 Python 测试均已用"改坏实现"验证过可复现失败，非恒真断言。
...
- **`run_live()` 新增 `session_tag=` 参数，支持多任务交易同一标的时的委托回报隔离**：0824 反馈第 1 点——平台多个任务交易**同一标的**时，各任务此前会收到彼此的 `on_order`/`on_trade`/`on_execution_report` 推送（v0.3.50 已解决「交易不同标的」的隔离，同标的这条此前判定为「需柜台在推送帧补任务标识才能解决」，该判定是错的）。条件其实已经具备：本框架生成的 `client_order_id` 格式是 `{broker}-{session_tag}-{run_salt}{seq}`，`session_tag` 默认随机 8 位 hex；中间件推送帧本就带 `client_order_id` 回传——比对前缀即可判定归属，**不需要改中间件**。传入 `session_tag`（推荐传平台任务 ID）后替代随机值作为前缀段，且**同时启用严格拒绝**：前缀不匹配的推送会被主动丢弃，计入新的 `foreign_task` 计数（与既有的 `foreign_symbol` 分开计，见下方 Changed）。校验 fail-fast：只允许 `[A-Za-z0-9_]`、长度 ≤ 32、不能含 `-`（它是 `client_order_id` 的字段分隔符），违反直接 `raise ValueError` 并给出可照抄的正确写法，不做静默归一化改写。
  ...
  自定义 broker 要支持该能力，必须在推送帧里**原样回传** `client_order_id`（不截断、不改写字符），详见 `docs/zh/advanced/adding_a_broker.md`「委托回报的标的过滤」一节；**内置的 `ctp` broker 不满足这个前提**——其 `client_order_id` 是本地 `order_ref` 反查出来的，不会发给柜台，外来单推回来时该字段恒为空，因此严格拒绝对 `ctp` 实际是空操作（不会报错，也不会拒绝任何外来单），传了 `session_tag` 也不会产生隔离效果，务必据此设定预期。
- **新增 `examples/70_csv_multi_symbol_import_demo.py`：从 CSV 文件导入多品种数据的回测示例**。补的是平台测试反馈里「品种通过文件导入需要补充示例」这一缺口——`docs/zh/guide/data.md` 早有「从 CSV 加载」的文字说明，但 `examples/` 73 个脚本里此前零个覆盖文件导入这条链路，尤其缺平台实际在用的那个固定 7 列格式（`date`/`symbol`/`open`/`high`/`low`/`close`/`volume`，`symbol` 去后缀纯数字、`date` 为按东八区解析后再去时区的 naive datetime、多标的 `pd.concat` 成一张表后 `run_backtest(symbols=[...])` 显式传白名单）。示例自带临时 CSV（跑完清理），覆盖读文件到回测的完整链路，并记录了一个实测坑：`pd.read_csv` 对纯数字 `symbol` 列（如 `"002202"`）默认推断成 `int64` 会丢前导 0，须显式 `dtype={"symbol": str}`。`docs/zh/guide/data.md`「2.1 从 CSV 加载」与 `examples/README.md` 已同步登记指向该示例。
...
- **实盘会话收尾摘要新增事件丢弃计数与被挡标的名单**：`Dropped (foreign symbol)` / `Dropped (duplicate order)` / `Dropped (foreign task)`，分别对应标的过滤、委托去重、会话标记严格拒绝丢掉的事件数。这些过滤本身按标的点名一次 WARNING 后就降级成 DEBUG，日志里很快看不见 —— 而万一标的匹配出错把**本会话自己的**回报也挡掉了，现象是「下单成功却收不到回调」这种极难排查的形态。计数挂在不会被日志级别关掉的地方，并在 `Dropped (foreign symbol)` 行后**列出被挡掉的标的代码**：三个计数在稳态下都会持续增长（计数点在事件入队处、早于去重，而柜台的全量 sync 返回全账户数据 ⇒ 同一笔委托每轮重复 +1），因此**数值大小不是故障判据**，判据是名单里有没有本任务挂载的标的。计数读取与格式化都做了异常隔离，不会因为一个可观测性字段而拖掉摘要主体（收益率、回撤、成交数）。
...
### Changed
- **委托回报的标的过滤新增会话标记判据，优先级高于原有的订单归属/标的判据**：`gateway/broker_event_bridge.py::_accepts_symbol` 之前只有两级（订单归属映射 → 标的白名单），现在前面加了两层，仅作用于 `order`/`trade`/`execution_report` 三类事件（`account` 事件没有 `client_order_id` 概念，不经过该判据，恒由原有逻辑放行）——`client_order_id` 前缀命中本会话 `{broker}-{session_tag}-` 一律放行（即便标的不在挂载列表内，覆盖 `BrokerOrderSink` 经外部信号源直调下单能合法报出挂载集合之外标的的场景），前缀不匹配且**显式传了 `run_live(session_tag=...)`**（严格模式）时主动拒绝，计入独立的 `foreign_task` 计数（与 `foreign_symbol` 分开计：前者是多任务稳态下必然增长的正常工作量，后者异常增长才指向标的配置错误）。**未传 `session_tag` 时严格拒绝完全不生效，但行为并非与升级前逐字一致**——前缀命中本会话时（第 1 层）仍会跳过标的判据直接放行，这在「自己的单 + 订单归属映射已被清理（如已 `pop`）+ 标的不在挂载集」这一具体组合下，会让升级前本会丢弃的回报改为放行；方向上只会让本会话更不容易丢自己的回报（不会更严格），但严格来说不是逐字不变。会话收尾摘要新增一行 `Dropped (foreign task)`；盘中周期性丢弃计数汇总（`_report_dropped_event_counts_if_changed`）的触发条件**不变**，仍只看 `foreign_symbol`，刻意不把 `foreign_task` 纳入，避免重现此前刚修掉的「稳态刷屏」。
...
### Fixed
- **（实盘）收到 `SIGTERM` 时也会触发 `on_stop`（平台「停止任务」按钮的常见实现）**：`run()` 的 `finally` 此前覆盖三条路径 —— 正常结束、`KeyboardInterrupt`（`Ctrl+C` / `duration` 到点）、异常中止；但全仓零个 `signal.signal` / `atexit`，平台若用 `terminate()` 停任务，进程直接死在默认 `SIGTERM` 处理里，`finally` 根本不执行，`on_stop` 里的撤单/平仓全部落空（0821 起的反馈，此前一直卡在「先问平台是怎么停的」上 —— 那个前置问题其实不必等：注册 handler 对 `SIGTERM` 通用有效，真正无解的只有 `kill -9`）。现在会话主循环期间注册 handler，把 `SIGTERM` 转成 `KeyboardInterrupt`，复用已被 `duration` 与 `Ctrl+C` 验证过的那条收尾路径 —— handler 里**只抛异常**，不跑用户的 `on_stop`（撤单、平仓、打摘要都得在正常栈上跑）。
  三处刻意的边界：① handler **只包主循环**，进入收尾就恢复原 handler —— 收尾期间再来一个 `SIGTERM` 按原行为处理（通常即终止），对齐「再按一次强制退出」的惯例，否则第二个信号会把撤单打断在半路；② 退出时**原样恢复**原 handler，`run_live` 可能被嵌进宿主进程，留着不还就是污染全局状态；③ 非主线程（平台用线程池承载多个任务）`signal.signal` 会抛 `ValueError`，此时只打一条 WARNING 并照常运行 —— 收不到 `SIGTERM` 收尾远好过让任务根本起不来。**仍然覆盖不到** `kill -9`（`SIGKILL`）与 Windows `TerminateProcess`：进程不经用户态代码即被回收，需要调度方改用优雅停止。
- **（实盘）盘中丢弃汇总不再稳态刷屏，也不再把排查引向一个不存在的归一化缺陷**：上面那条盘中汇总（挂在全量 sync 那一档）此前用 `foreign_symbol` 的**计数增量**作触发条件，文案还断言「若配置正确应恒为 0，出现增长请怀疑标的归一化把自己的回报也挡掉了」—— 两处都是错的。该计数的计数点在事件入队处、**早于一切去重**，而柜台的 `sync_open_orders()` / `sync_today_trades()` 返回的是全账户数据 ⇒ 同账户下只要存在非挂载标的的挂单（别的任务、人工下单终端、隔夜挂单），同一笔每轮就 +1；且未传 `session_tag` 时**不启用**严格任务隔离，别的任务的委托不走 `foreign_task` 而是落进 `foreign_symbol`。于是它在默认配置下必然线性增长，日志每 30 秒准时刷一条 —— 与当初要修的「挂单每秒被重复推送」是同形的失败模式（同一个函数里已经为 `duplicate_order` 逐字论证过这一点，却对 `foreign_symbol` 犯了同样的错）；而那句文案会让人去查一个并不存在的缺陷（2026-08-26 平台测试反馈原文就是照抄它在怀疑）。
  现在触发条件改为「**被挡掉的标的集合出现新成员**」，与逐条丢弃日志（首次 WARNING 点名、之后同标的降 DEBUG）同一套防刷屏口径：稳态完全静默，出现新的外来标的才报一条、且只点名新出现的那个。两个累计值退化成日志里的上下文，并明确写出「会随每轮全量 sync 对同一笔委托重复累加，不代表故障；仅当点名的标的**是本任务挂载的标的**时，才说明标的匹配或配置有问题」。`BrokerEventBridge` 新增 `dropped_foreign_symbol_names()` 提供被挡标的名的快照（返回副本，避免调用方的集合运算污染防刷屏状态），会话收尾摘要据此在 `Dropped (foreign symbol)` 行后列出名单。该集合的读写一并纳入事件锁：它现在由 recovery 线程读、行情/推送线程写，无锁复制会在「复制期间另一线程 add」时抛 `Set changed size during iteration`；顺带修掉一处既有竞态 —— 并发下同一标的此前可能各打一条首次 WARNING。
