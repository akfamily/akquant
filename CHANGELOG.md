...
- 修复纯 tick 回测下 `ClosedTrade.mae` / `mae_pct` / `mfe` / `mfe_pct` / `max_drawdown_pct` 静默恒为 0 的问题（回归修复）。tick/bar 历史序列拆分后，引擎内部计算这几个指标时仍只读 bar 容器，纯 tick 回测下该容器恒为空，取不到历史后按初值 0 写入成交记录，且不报错——若你此前跑过纯 tick 回测（`run_backtest(data=[Tick, ...])`，未配合 `freq` 聚合出 bar），报告里这几列为 0 很可能是本 bug 所致，并非策略本身盈亏波动为零，建议重新跑一次核对。混合输入（tick + bar 同时存在）与纯 bar 回测不受影响，golden 基线未变化。
